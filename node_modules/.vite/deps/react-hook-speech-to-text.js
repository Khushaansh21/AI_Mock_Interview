...
l.mixin = function(e2) {
  ...
  t2.isWildEmitter = true, t2.on = function(e3, t3, n2) {
    ...
    var t3, n2, o2, r2;
    for (t3 in this.callbacks = this.callbacks || {}, this.callbacks) for (n2 = 0, o2 = (r2 = this.callbacks[t3]).length; n2 < o2; n2++) r2[n2]._groupName === e3 && (r2.splice(n2, 1), n2--, o2--);
    return this;
    ...
    if (i2) for (t3 = 0, n2 = (o2 = i2.slice()).length; t3 < n2 && o2[t3]; ++t3) o2[t3].apply(this, r2);
    if (a2) for (n2 = a2.length, t3 = 0, n2 = (o2 = a2.slice()).length; t3 < n2 && o2[t3]; ++t3) o2[t3].apply(this, [e3].concat(r2));
    return this;
  }, t2.getWildcardCallbacks = function(e3) {
    this.callbacks = this.callbacks || {};
    var t3, n2, o2 = [];
    for (t3 in this.callbacks) n2 = t3.split("*"), ("*" === t3 || 2 === n2.length && e3.slice(0, n2[0].length) === n2[0]) && (o2 = o2.concat(this.callbacks[t3]));
    return o2;
  };
...
function d(e2, t2) {
  var n2, o2 = this;
  if (t2 = t2 || {}, h) return n2 = e2.toString().trim().match(/^function\s*\w*\s*\([\w\s,]*\)\s*{([\w\W]*?)}$/)[1], new f.Worker(f.URL.createObjectURL(new f.Blob([n2], { type: "text/javascript" })));
...
var g = class {
  constructor(e2, t2) {
    this.config = { bufferLen: 4096, numChannels: 1, mimeType: "audio/wav", ...t2 }, this.recording = false, this.callbacks = { getBuffer: [], exportWAV: [] }, this.context = e2.context, this.node = (this.context.createScriptProcessor || this.context.createJavaScriptNode).call(this.context, this.config.bufferLen, this.config.numChannels, this.config.numChannels), this.node.onaudioprocess = (e3) => {
      if (this.recording) {
        for (var t3 = [], n2 = 0; n2 < this.config.numChannels; n2++) t3.push(e3.inputBuffer.getChannelData(n2));
    ...
    this.worker = new p(function() {
      ...
      function i2() {
        for (let e4 = 0; e4 < t3; e4++) r2[e4] = [];
      }
      function a2(e4, t4) {
        let n3 = new Float32Array(t4), o3 = 0;
        for (let t5 = 0; t5 < e4.length; t5++) n3.set(e4[t5], o3), o3 += e4[t5].length;
        return n3;
      }
      function s2(e4, t4, n3) {
        for (let o3 = 0; o3 < n3.length; o3++) e4.setUint8(t4 + o3, n3.charCodeAt(o3));
      }
      this.onmessage = function(c2) {
        switch (c2.data.command) {
          ...
          case "record":
            !function(e4) {
              for (var n3 = 0; n3 < t3; n3++) r2[n3].push(e4[n3]);
              o2 += e4[0].length;
            }(c2.data.buffer);
            break;
          case "exportWAV":
            !function(i3) {
              ...
              let u2 = function(e4) {
                ...
                if (n3 == e3) return t4;
                if (n3 > e3) throw "downsampling rate show be smaller than original sample rate";
                var o3 = e3 / n3, r3 = Math.round(t4.length / o3), i4 = new Float32Array(r3), a3 = 0, s3 = 0;
                for (; a3 < i4.length; ) {
                  for (var c4 = Math.round((a3 + 1) * o3), l4 = 0, u3 = 0, f3 = s3; f3 < c4 && f3 < t4.length; f3++) l4 += t4[f3], u3++;
          ...
          case "getBuffer":
            !function() {
              let e4 = [];
              for (let n3 = 0; n3 < t3; n3++) e4.push(a2(r2[n3], o2));
  ...
  getBuffer(e2) {
    if (!(e2 = e2 || this.config.callback)) throw new Error("Callback not set");
    this.callbacks.getBuffer.push(e2), this.worker.postMessage({ command: "getBuffer" });
  }
  exportWAV(e2, t2) {
    if (t2 = t2 || this.config.mimeType, !(e2 = e2 || this.config.callback)) throw new Error("Callback not set");
        ...
        switch (r2.label) {
            ...
            return !S && y ? (function() {
              if (y) {
                ...
                t3 && (y.grammars = t3), o3 && (y.lang = o3), y.interimResults = n3 || false, y.maxAlternatives = r3 || 1, y.start(), y.onresult = function(e4) {
                  ...
                  if (n3) if (t4.isFinal) _(void 0), D(function(e5) {
                    return a(e5, [{ transcript: o4, timestamp: r4 }]);
                  }), O(function(e5) {
                    return a(e5, [o4]);
                  });
                  else {
                    for (var i2 = "", s2 = e4.resultIndex; s2 < e4.results.length; s2++) i2 += e4.results[s2][0].transcript;
                    _(i2);
                  }
                  else D(function(e5) {
                    return a(e5, [{ transcript: o4, timestamp: r4 }]);
          ...
          case 1:
            return e2 = r2.sent(), B(true), R2 && (clearTimeout(G.current), $()), J.current && X(), J.current = e2.clone(), (t2 = function(e3, t3) {
              ...
              for (var g3 = 0; g3 < d3; g3++) n3.speakingHistory.push(0);
              var m3 = function() {
                setTimeout(function() {
                  if (p3) {
                    var e4 = function(e5, t5) {
                      ...
                      for (var o5 = 4, r4 = t5.length; o5 < r4; o5++) t5[o5] > n4 && t5[o5] < 0 && (n4 = t5[o5]);
                      return n4;
                    }(i2, r3);
                    n3.emit("volume_change", e4, f3);
                    var t4 = 0;
                    if (e4 > f3 && !n3.speaking) {
                      for (var o4 = n3.speakingHistory.length - 3; o4 < n3.speakingHistory.length; o4++) t4 += n3.speakingHistory[o4];
                      t4 >= 2 && (n3.speaking = true, n3.emit("speaking"));
                    } else if (e4 < f3 && n3.speaking) {
                      for (o4 = 0; o4 < n3.speakingHistory.length; o4++) t4 += n3.speakingHistory[o4];
